...
class MapPlotter(Node):
    ...
    def callback(self, scan_msg, odom_msg):
        ...
        oy = -1 * np.sin(angles) * ranges
        ox = np.cos(angles) * ranges
        ox = self.clean_data(ox)
        oy = self.clean_data(oy)
    ...
    def generate_ray_casting_grid_map(self,ox, oy, xy_resolution, breshen=True):
        ...
        # Se o mapa de ocupação não existir, inicialize-o
        ...
        if breshen:
            for (x, y) in zip(ox, oy):
                ...
                for laser_beam in laser_beams:
                    if 0 <= laser_beam[0] < self.occupancy_map.shape[0] and 0 <= laser_beam[1] < self.occupancy_map.shape[1]:
                        self.occupancy_map[laser_beam[0]][laser_beam[1]] = 0.0
                self.occupancy_map[ix][iy] = 1.0
                self.occupancy_map[ix + 1][iy] = 1.0
                self.occupancy_map[ix][iy + 1] = 1.0
                self.occupancy_map[ix + 1][iy + 1] = 1.0
        else:
            self.occupancy_map = self.init_flood_fill((center_x, center_y), (ox, oy), (x_w, y_w), (min_x, min_y), xy_resolution)
            self.flood_fill((center_x, center_y), self.occupancy_map)
            self.occupancy_map = np.array(self.occupancy_map, dtype=float)
            for (x, y) in zip(ox, oy):
                ix = int(round((x - min_x) / xy_resolution))
                iy = int(round((y - min_y) / xy_resolution))
                self.occupancy_map[ix][iy] = 1.0
        ...
        return self.occupancy_map, min_x, max_x, min_y, max_y, xy_resolution
